...
class Cortex(object):
    ...
    async def send_command(self, method, auth=True, callback=None, **kwargs):
        ...
        if 'error' in resp:
            ...
            if resp['error']['code'] == -32015:
                logger.warning(f"Sending authorization command")
                await self.authorize(debit=2)
            else:
    ...
    async def authorize(self, license_id=None, debit=None):
        ...
        with open('./CortexInfo/authorizationToken.txt','w') as f1:
            f1.write(resp['result']['cortexToken'])
    ...
    async def has_access_right(self):
        params = {'clientId': self.client_id,
                  'clientSecret': self.client_secret}
        resp = await self.send_command('hasAccessRight', auth=False, **params)
        logger.debug(f"{__name__} resp:\n{resp}")
    ...
    async def inject_marker_modified(self,label='', value=0, port='', time=None):
        if not time:
            time = datetime.now()
        ...
        msg = self.gen_request('injectMarker', self.auth_token, **params)
        await self.websocket.send(msg)
