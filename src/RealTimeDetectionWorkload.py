...
class DataProducer(Thread):
    ...
    def __init__(self, controller, data_queue):
        ...
        normalizationValues = pd.read_csv('./Model/modelNBackNormalization.csv', sep=',')
    ...
    def run(self):
        ...
        self.predictionModel = load_model('./Model/modelNBack.h5')
        # time.sleep(5)
        ...
        if len(cortex.headsets) > 0:
            ...
            try:
                while self.controller.running:
                    ...
                    #Roll Oldest data to the last row
                    self.sequenceForPrediction = np.roll(self.sequenceForPrediction, -1, axis=1)
                    #Set new data point in last row
                    self.sequenceForPrediction[0,-1,:] = dataPoint

                    #print(self.sequenceForPrediction)

                    # Make Prediction
                    prediction = self.predictionModel.predict(self.sequenceForPrediction)
