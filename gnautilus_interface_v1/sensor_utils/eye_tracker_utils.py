...
def create_eye_tracker_features(gpx_arr, gpy_arr, pd_arr,lsl_time_arr):
    detector = fixation_detector(gpx_arr, gpy_arr, lsl_time_arr * 1000)
    fix_array = detector.fixation_dection(mindur=85)
    fix_array = np.array(fix_array)

    fix_pos = fix_array[:, [3, 4]]
    print(fix_array.shape)
    nni = NNI(fix_pos, (1, 1))
    ...
    results = pd.DataFrame(np.array([numb_of_fixations,average_fix,ssp,nni_value, mean_pd]).reshape(1,5)
                           ,columns=["number_of_fix","average_fix","ssp","nni_value","pd"])
    return results