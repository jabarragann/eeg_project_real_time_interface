...
from gnautilus_interface_master_thesis.DataProducerEyeTracker import DataProducer
from gnautilus_interface_master_thesis.LivePlotsV2 import LivePlots
from gnautilus_interface_master_thesis.AlarmModule import AlarmModule, FeedbackModule
from gnautilus_interface_master_thesis.OddballModule import OddBallModule
from gnautilus_interface_master_thesis.SuctionAssistantModule import SuctionAssistantModule
import queue
    ...
    def listen_threshold_markers(self):
        ...
        while self.running:
            ...
            if len(streams) > 0 :
                ...
                print("Threshold inlet cmd found")
                while self.running:
                    sample, timestamp = inlet.pull_sample(timeout=1)
                    if timestamp is not None:
                        print("got new threshold cmd %s at time %s" % (sample[0], timestamp))
                        cmd = str(sample[0])
                        if cmd == "u":
